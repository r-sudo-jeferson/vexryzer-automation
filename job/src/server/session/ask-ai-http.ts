...
import {
  ...
  type StoredUserCorrectionDependencies,
  type StoredUserCorrectionResult,
} from './stored-user-correction-service.ts';
...
export interface AskAiHttpDependencies {
  ...
  correctionDependencies?: Partial<StoredUserCorrectionDependencies>;
}
...
const CLOSED_CORRECTION_KEYS = new Set(['sessionId', 'requestId', 'expectedRevision', 'correctionId']);
...
function mapCorrectionFailure(
  result: Exclude<StoredUserCorrectionResult, { ok: true }>,
): Response {
  switch (result.code) {
    case 'INVALID_REQUEST':
...
export async function handleAskAiCorrection(
  request: Request,
  dependencies: Readonly<AskAiHttpDependencies>,
): Promise<Response> {
  if (request.method !== 'POST') return methodNotAllowed();
  ...
  if (!parsed.ok) return parsed.response;
  if (!isRecord(parsed.value)
    || Object.keys(parsed.value).some((key) => !CLOSED_CORRECTION_KEYS.has(key))
    || ![...CLOSED_CORRECTION_KEYS].every((key) => Object.hasOwn(parsed.value as object, key))) {
    return json({ ok: false, code: 'INVALID_REQUEST' }, 400);
  }

  const { sessionId, requestId, expectedRevision, correctionId } = parsed.value;
  if (
    typeof sessionId !== 'string'
    || typeof requestId !== 'string'
    || typeof expectedRevision !== 'number'
