...
- Mistral-backed Experience Orchestrator behind a provider-neutral server seam;
...
- prove the selected pinned DeepSeek Harness version with the real selected Mistral route/model;
...
18. DeepSeek Harness + Mistral compatibility is proven on an exact pinned version before production dependency commitment.
