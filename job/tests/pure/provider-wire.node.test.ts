...
import { ChatSseDecodeError, ChatStreamChunkError, createChatSseDecoder, createChatStreamAccumulator } from '../../src/server/ai/providers/chat-sse.ts';
...
test('SSE decoder fails closed on malformed JSON, non-standard fields or oversized unframed input', () => {
  ...
  assert.throws(
    ...
    (error: unknown) => error instanceof ChatSseDecodeError && error.code === 'malformed_json',
  );
  ...
  assert.throws(
    () => huge.push('data: ' + 'x'.repeat(40)),
    (error: unknown) => error instanceof ChatSseDecodeError && error.code === 'buffer_limit',
  );
});
...
test('DONE is terminal even when malicious data follows in the same network chunk', () => {
  ...
  assert.throws(
    ...
    (error: unknown) => error instanceof ChatSseDecodeError && error.code === 'data_after_done',
  );
});
